...
class Institution {
        ...
        if (student.userName in this.studentList) {
          if (offering.registeredStudents.includes(student)) {
            ...
            if (offering.timeSlot) {
              for (const enrolled of student.courseList) {
                if (
                  enrolled.timeSlot &&
                  this.timeSlotsOverlap(offering.timeSlot, enrolled.timeSlot)
                ) {
  ...
  timeSlotsOverlap(slotA, slotB) {
    // Check for shared day
    const hasSharedDay = slotA.days.some((day) => {
      return slotB.days.includes(day);
    });
    ...
    const slotAStart = convertToMinutes(slotA.startTime);
    const slotAEnd = convertToMinutes(slotA.endTime);
    const slotBStart = convertToMinutes(slotB.startTime);
    const slotBEnd = convertToMinutes(slotB.endTime);

    // Check for overlap
    const startsBeforeOtherEnds = slotAStart < slotBEnd;
    const endsAfterOtherStarts = slotBStart < slotAEnd;
    ...
    if (dept) {
      ...
      if (Object.keys(this.courseSchedule).length) {
        for (const offerings of Object.values(this.courseSchedule)) {
          const filtered = offerings.filter(
            (offering) =>
              offering.year === year && offering.quarter === quarter,
          );
          filtered.forEach((item) => schedule.push(item.toString()));
        }
        if (schedule.length) {
